#!/usr/bin/env python3
"""
Convert Ukrainian V2c CSV format to PyLaia format.

Input:
- data/pylaia_ukrainian_v2c_train/train.csv
- data/pylaia_ukrainian_v2c_val/train.csv
...
def main():
    # Paths
    train_csv = Path('data/pylaia_ukrainian_v2c_train_fresh/train.csv')
    val_csv = Path('data/pylaia_ukrainian_v2c_val_fresh/train.csv')
    output_dir = Path('data/pylaia_ukrainian_v2c_combined')
    ...
    for _, row in train_df.iterrows():
        img_path = f"../pylaia_ukrainian_v2c_train_fresh/{row['image_path']}"
        text = row['text']
        lines_train.append(f"{img_path} {text}")

    lines_val = []
    for _, row in val_df.iterrows():
        img_path = f"../pylaia_ukrainian_v2c_val_fresh/{row['image_path']}"
        text = row['text']
