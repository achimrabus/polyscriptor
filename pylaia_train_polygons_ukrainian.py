"""
Train PyLaia CRNN model for Ukrainian dataset with PAGE XML extracted lines.
...
def main():
    # Configuration for Ukrainian dataset with PAGE XML extracted lines
    config = {
        'train_dir': 'data/pylaia_ukrainian_pagexml_train',
        'val_dir': 'data/pylaia_ukrainian_pagexml_val',
        'output_dir': 'models/pylaia_ukrainian_pagexml',
        'img_height': 64,  # Matching extraction height
        'batch_size': 32,  # Optimized for single RTX 4090
        'num_workers': 8,
        'cnn_filters': [16, 32, 64, 64],  # Slightly larger for 64px height
        ...
        'early_stopping': 25,  # Patience increased
        'augment': True,
        'use_multi_gpu': False
    }
    ...
    logger.info("\n" + "="*60)
    logger.info("TRAINING WITH PAGE XML EXTRACTED LINES")
    logger.info("="*60)
    logger.info("Benefits:")
    logger.info("  ✓ Polygon-based line extraction (not rectangles)")
    logger.info("  ✓ Baseline-based deskewing")
    logger.info("  ✓ Proper text region masking")
    logger.info("  ✓ Height normalization")
    logger.info("="*60 + "\n")
    ...
    logger.info(f"Expected improvement: 20% CER → <10% CER")
    logger.info("="*60 + "\n")
    ...
    logger.info(f"Expected CER: <10% (vs ~20% with simple extraction)")
    logger.info("\nNext steps:")
    logger.info("  1. Check validation CER in training logs")
    logger.info("  2. Test on sample images")
    logger.info("  3. Add language model for further improvement")
    logger.info("="*60)
