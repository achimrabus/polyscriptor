...
# just to make the path to the model files shorter
root: str = "C:/Users/dhlabadmin/Desktop/m-test/pylaia-iam/"

with open(root+"my_model/model_config.json", "r") as f:
    cfg = json.load(f)
...
with open(root+"my_model/model_config.json") as f:
    cfg = json.load(f)
...
image = load_image("000000.png", cfg["img_height"]).to(device)
with torch.no_grad():
